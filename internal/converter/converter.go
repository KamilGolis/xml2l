...
// Convert discovers all *.profile-meta.xml files under projectPath, converts
// each, and returns a map of output path → YAML content.
func Convert(projectPath string) (map[string][]byte, error) {
	profiles, err := findProfiles(projectPath)
	if err != nil {
		return nil, err
	}

	results := make(map[string][]byte, len(profiles))
	for _, path := range profiles {
		outPath, yamlBytes, err := ConvertFile(path)
		if err != nil {
			return nil, err
		}
